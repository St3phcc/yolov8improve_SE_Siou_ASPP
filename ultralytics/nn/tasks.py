...
from .modules.transformer import AIFI, DeformableTransformerDecoder, TransformerFPN
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m is AIFI:
            c1 = ch[f]  # 输入通道来自上一层
            args = [c1, *args]  # 参数格式: [输入通道, embed_dim, num_heads, ...]
    
        # 新增：处理 DeformableTransformerDecoder
        elif m is DeformableTransformerDecoder:
            args = [ch[f]] + args  # 输入通道 + 配置参数
    
        # 新增：处理 TransformerFPN
        elif m is TransformerFPN:
            # 输入来自多个层（如 [[-1, -3, -5], ...]）
            c1 = [ch[x] for x in f]  # 收集多个输入通道
            args = [c1, *args]       # 参数格式: [输入通道列表, 输出通道]
