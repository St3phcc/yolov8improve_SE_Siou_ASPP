...
class TransformerFPN(nn.Module):
    def __init__(self, in_dims, out_dim=256):
        ...
        self.proj_layers = nn.ModuleList([
            nn.Conv2d(dim, out_dim, 1) for dim in in_dims
        ])
        self.fusion = TransformerEncoderLayer(out_dim, num_heads=8)
    
    def forward(self, feats):
        # 特征投影到统一维度
        proj_feats = [proj(f) for proj, f in zip(self.proj_layers, feats)]
        # Transformer特征融合
        fused = [self.fusion(f.flatten(2).permute(0,2,1)) for f in proj_feats]
        return fused
...
class AIFI(TransformerEncoderLayer):
    ...
    def __init__(self, c1, cm=2048, num_heads=8, dropout=0, act=nn.GELU(), normalize_before=False):
        ...
        self.channel_adjust = Conv(c1, cm) if c1 != cm else nn.Identity()
